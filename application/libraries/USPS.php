...
class USPS {
    ...
    var $user_id = '289UNICA2637';
    var $secure = TRUE;
    var $test = TRUE;
    var $host = 'secure.shippingapis.com';
    var $secure_host = 'secure.shippingapis.com';
    var $test_api = 'ShippingAPI.dll';
     ...
     */
    function __constructor ($config = array()) {
    ...
    function shipping_rate_lookup($packages = array()) {
        ...
        foreach ($packages as $package) {
            ...
            if (!empty($package['Machinable']) && $package['Machinable'] != 0) {
    ...
    function get_shipping_label($from, $to, $weight_in_ounces, $service_type, $dimensions = array(), $container = "VARIABLE", $size = "REGULAR", $insured_amount = 0) {
        ...
        if ($insured_amount > 0)
            $xml .= '<InsuredAmount>' . $insured_amount . '</InsuredAmount>';
        ...
        $xml = '<DelivConfirmCertifyV4.0Request USERID="">
                <Option>1</Option>
                <ImageParameters>
                <LabelSequence>
